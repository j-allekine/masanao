...
└── Activity
    ...
    └── Meal Schedule
        └── Schedule Entry (canonical internal term; final user-facing label under review)
            ├── User-entered name / description, e.g. Lunch, AM Snack, or Feeding
            ├── User-selected time
            └── Issuance Record (exactly one when supply lines exist; may be drafted before posting)
                ├── Food Supply
...
An Activity Design can contain multiple Activities. “Visitors” and “Barangay Feeding” are examples of Activity names, not a visitor entity. An Activity has exactly one date and one Meal Schedule container, which can contain multiple schedule entries. A schedule entry is named and described by the user, and its time is selected by the user. Lunch, Snack, and Feeding are examples, not a required enum or fixed list.
...
A schedule entry may start empty. Once it contains supply lines, it has exactly one Issuance Record, which may be drafted before posting. The record may contain multiple food supply lines with editable ingredient issue lines and direct-use stock issue lines.

## POS-like issuance workspace

The issuance part of an Activity should feel like a simple POS-like page:

1. The user opens a schedule entry and adds foods or direct-use items.
2. A food cannot be added without a Recipe. Each food's default ingredient rows appear in an editable table.
3. The user can edit ingredient quantities and add more ingredient rows for that entry.
...
7. Once the schedule entry contains supply lines, it has exactly one Issuance Record, which can remain a draft until it is posted.

The master Recipe remains a reusable reference with multiple ingredient lines. Selecting a Recipe copies its current ingredient rows into the Schedule Entry's Recipe Snapshot. Those copied rows become editable ingredient issue lines: they may be adjusted or extended without changing the master Recipe, and later Recipe edits do not change the snapshot or posted Issuance history. How Recipes created inline are retained and reused later remains a small open note.
...
Activity Design, Activity, Activity particulars, Meal Schedule, Schedule Entries, Food Supply, and Recipe are planning or reference information. They do not, by themselves, create inventory movements.

Posting a Delivery Receipt creates inventory stock-in movements for the delivered quantities. A schedule entry may have no supply lines or Issuance Record at first. Once it contains supply lines, it has exactly one Issuance Record. A draft Issuance Record has no inventory effect; posting the issuance creates ledger movements for the actual quantities and batches issued through editable ingredient issue lines and direct-use stock issue lines. One ingredient may be issued from multiple inventory batches.

## Source alignment note

The accepted local decisions for this workflow are [ADR 0002](adr/0002-activity-schedule-and-issuance-boundary.md), which records the Activity, Meal Schedule, Schedule Entry, and Issuance boundary, and [ADR 0003](adr/0003-partial-deliveries-and-immediate-stock-in.md), which records partial deliveries and immediate stock-in. The older source-repo ADR 0003 at `C:\Users\LENOVO\Code\lgu-san-miguel\masanao-system\docs\adr\0003-activity-design-issuance-and-nonstock-food-output.md` remains historical and unmodified.

## Unresolved questions

1. What user-facing label should be shown for the canonical **Schedule Entry** term?

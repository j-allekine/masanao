...
describe("Delivery Receipt direct Base Unit posting", () => {
  ...
  it("stocks the actual received quantity and requires a note only for a true numeric variance", async () => {
    const { purchaseOrderId, itemId } = await receiptContext("variance-vendor");
